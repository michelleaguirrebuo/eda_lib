...
class EDA:
    ...
    def __init__(self, file:str='', sheet:str or int=0, set_df:bool=False, df=None):
        ...
        - sheet: 0 by default. Sheet name in case of an excel file w/
          multiple sheets. Follows pandas syntax.
        - set_df: false by default. Boolean value to indicate if a custom
          dataframe is being inputted instead of a file
        - df: None by default. Custom dataframe.
        - 
        '''
    ...
    def addTruncatedColumn(self, column:str, truncateAt:int or str=11, last:bool=False):
        df=self.df[column].apply(lambda x: str(x))
        if type(truncateAt)==int:
          self.df[column+f'_{'last' if last else 'First'}{truncateAt}']=df.apply(lambda x: x[:truncateAt] if not last else x[truncateAt:])
          return self.df[column+f'_{'last' if last else 'First'}{truncateAt}']
        if type(truncateAt)==str:
          self.df[column+f'_{'after_' if last else 'before_'}{truncateAt}']=df.apply(lambda x: x.split(truncateAt)[0] if not last else x.split(truncateAt)[1])
          return self.df[column+f'_{'after_' if last else 'before_'}{truncateAt}']
